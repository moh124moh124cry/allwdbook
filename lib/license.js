...
function licenseSecret() {
  const value = String(process.env.LICENSE_SECRET || "");
...
export function normalizeLicenseCode(value) {
  ...
  if (!compact.startsWith("AWDLIFE")) return "";

  const body = compact.slice(7);

  if (body.length !== CODE_GROUPS * CODE_GROUP_SIZE) return "";

  const groups = body.match(
    new RegExp(`.{1,${CODE_GROUP_SIZE}}`, "g"),
  );

  return groups ? `AWD-LIFE-${groups.join("-")}` : "";
}
...
export function decryptLicenseCode(ciphertext) {
  const [ivValue, tagValue, encryptedValue] = String(
    ciphertext || "",
  ).split(".");

  if (!ivValue || !tagValue || !encryptedValue) {
    throw new Error("INVALID_LICENSE_CIPHERTEXT");
  }
...
export function hashEmailOtp({
  ...
  return crypto
    .createHmac("sha256", licenseSecret())
    .update(
      `${licenseId}:${String(email).toLowerCase()}:${purpose}:${otp}`,
    )
    .digest("hex");
}

export function safeEqualHex(first, second) {
...
export function acceptsTestMode(testMode) {
  return (
    testMode !== true ||
    process.env.LEMONSQUEEZY_ACCEPT_TEST_MODE === "true"
  );
}
...
export async function createLifetimeLicense(
  ...
  for (let attempt = 0; attempt < 4; attempt += 1) {
    ...
    const record = {
      ...
      code_ciphertext: encryptLicenseCode(code),
      ...
      max_activations: Math.max(
        1,
        Math.min(10, Number(maxActivations) || 3),
      ),
      lemon_order_id: lemonOrderId || null,
      lemon_customer_id: lemonCustomerId || null,
      lemon_product_id: lemonProductId || null,
      lemon_variant_id: lemonVariantId || null,
    ...
    if (error.code !== "23505") {
      throw new Error(
        `LICENSE_CREATE_FAILED:${error.message}`,
      );
    }
  }

  throw new Error("LICENSE_CODE_COLLISION");
}
...
export async function activateLifetimeLicense(
  ...
  if (existingError) {
    throw new Error(
      `ACTIVATION_LOOKUP_FAILED:${existingError.message}`,
    );
  }
  ...
  if (existing) {
    ...
    if (error) {
      throw new Error(
        `ACTIVATION_UPDATE_FAILED:${error.message}`,
      );
    }
  ...
  if (countError) {
    throw new Error(
      `ACTIVATION_COUNT_FAILED:${countError.message}`,
    );
  }
  ...
  if (error) {
    throw new Error(
      `ACTIVATION_CREATE_FAILED:${error.message}`,
    );
  }
